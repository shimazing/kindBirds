...
class Replay(object):
  def __init__(self, memory_size, observation_dims=[15], alpha=0.7, beta=0.5):
    # memory
    self.prebird = np.empty([self.memory_size, 3], dtype=np.float)
    self.postbird = np.empty([self.memory_size, 3], dtype=np.float)
  ...
  def sample_one(self):
    ...
    # update
    # TODO : solve unresolved reference
    # update_priority = self.pred_net.get_priority(observed, bird, action, target) # no reference to pred_net, target
